#!/usr/bin/env python3
"""Dance test node — runs the dance once and stands down.

WATCH: the robot should sink into a slight crouch and groove with all six
feet planted — bounce, side-sway with a lean, a circling grind, a twist, then
a bow and a quick pop — and rise back to standing. No foot should ever leave
or slide on the ground, and every move should flow into the next without a
stop. About eighteen seconds.

The routine validates every frame offline before moving (nodes/dance.py),
and the hardware node refuses it below the battery floor.
"""
...
ABORT_V = float(os.environ.get("PIBOT_DANCE_ABORT_V", "4.9"))
STEP_TIMEOUT_S = 90.0

# dance already ends standing in neutral; relax is the safe tail.
STEPS = [
    ("dance", "dance", {}),
